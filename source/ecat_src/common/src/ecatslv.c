...
#endif //#if CHECK_SM_PARAM_ALIGNMENT
#else
...
#endif //CHECK_SM_PARAM_ALIGNMENT
#else
    SMLength = pSyncMan->Length;
    SMAddress = pSyncMan->PhysicalStartAddress;
...
#if (MAX_PD_OUTPUT_SIZE > 0) && ((MAILBOX_SUPPORTED) || (MAX_PD_INPUT_SIZE > 0))
    if (
#if MAILBOX_SUPPORTED
    ...
    */
    ...
    if((dcControl & (ESC_DC_SYNC_UNIT_ACTIVE_MASK | ESC_DC_SYNC_UNIT_AUTO_ACTIVE_MASK)) != 0)
    {
...

        {
            ...
            curMinCycleTime = sSyncManInPar.u32MinCycleTime;
#endif
#endif
...
/*ECATCHANGE_START(V5.13) ESM2*/
        ...
        case PREOP_2_INIT:
            ...
            break;
#else
/*ECATCHANGE_START(V5.13) BOOT1*/
        case INIT_2_INIT:
            BackToInitTransition();
            result = NOERROR_NOSTATECHANGE;
            break;
/*ECATCHANGE_END(V5.13) BOOT1*/
#endif
#if !BOOTLOADER_SAMPLE
        case INIT_2_INIT:
            BackToInitTransition();
...
#elif(MAX_PD_INPUT_SIZE > 0)
            if ((sErrorSettings.u16SyncErrorCounterLimit == 0) || (sSyncManInPar.u16SmEventMissedCounter < sErrorSettings.u16SyncErrorCounterLimit))
#endif
...
/*ECATCHANGE_END(V5.13) MBX1*/
...
#if COE_SUPPORTED && TEST_APPLICATION
    if (IS_TEST_ACTIVE(ESM0_PENDING_TRANSITIONS))
    {

        /* complete pending ESM transition */
        if (bPendingESMLock)
        {
    ...
    {
        if (u16PsTransitionCallsHelper == 1)
        {

#ifdef DebugMessage
            DebugMessage("Pend ESM PreOP 0x52\n", nEcatStateTrans);
#endif
            ECAT_StateChange(STATE_PREOP, ALSTATUSCODE_EXT_HARDWARE_NOT_READY);
            u16PsTransitionCallsHelper++;
        }
        else if(u16PsTransitionCallsHelper == 3)
        {
#ifdef DebugMessage
            DebugMessage("Pend ESM OK\n", nEcatStateTrans);
#endif
            ECAT_StateChange(0, 0);
            INC_TEST_CNT(ESM0_PENDING_PS_TRANSITION_WITH_ERROR);
            u16PsTransitionCallsHelper++;
        }
    }
#endif
...
#if TEST_APPLICATION
#if COE_SUPPORTED
        if (SET_TEST_REQUEST_PENDING(EscAlControl))
        {
            UpdateActiveTest();
        }
        else
#endif // #if COE_SUPPORTED
            if (SET_CTT_TEST_REQUEST_PENDING(EscAlControl))
        {
            HandleCttTestCaseID();
        }
        else
...
/*ECATCHANGE_END(V5.13) MBX1*/
...
#if TEST_APPLICATION
        if (IS_TEST_ACTIVE(Mbx_IgnoreMbxRepeatRequest) == FALSE)
        {
#endif //#if TEST_APPLICATION
        ...
        {
            INC_TEST_CNT(Mbx_IgnoreMbxRepeatRequest)
        }
#endif //#if TEST_APPLICATION
