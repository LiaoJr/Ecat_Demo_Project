...
#if (EL9800_APPLICATION + CiA402_SAMPLE_APPLICATION  + TEST_APPLICATION + SAMPLE_APPLICATION + SAMPLE_APPLICATION_INTERFACE + BOOTLOADER_SAMPLE)  > 1
#error Select only one application.
#endif

#if APPLICATION_FILE && BOOTLOADER_SAMPLE
#error Select only one application.
#endif
...
#if BOOTLOADER_SAMPLE && !BOOTSTRAPMODE_SUPPORTED
#error The Bootstrap mode shall also be supported if the Boot loader sample is enabled.
#endif
...
#if CiA402_SAMPLE_APPLICATION && !COE_SUPPORTED
...
#if CiA402_SAMPLE_APPLICATION && SOE_SUPPORTED
